"""Round 19 - bidirectional AttentionLSTM (TBAL paper direction) on full set-a.

Mirrors Zheng et al. BiLSTM + attention on PhysioNet 2012: same original
stride-15 val split + set-b holdout gates. Saves checkpoints + logits for the
ensemble pool. No deploy here (selection happens after).
"""
import datetime
import json
import os
...
CONFIGS = [
    ('bidir-lr1e4-s6', dict(lr=1e-4, wd=1e-4, step_every=6, seed=61, hidden=96, dropout=0.3)),
    ('bidir-lr2e4-s6', dict(lr=2e-4, wd=1e-4, step_every=6, seed=62, hidden=96, dropout=0.3)),
    ('bidir-lr1e4-do4', dict(lr=1e-4, wd=1e-4, step_every=6, seed=63, hidden=96, dropout=0.4)),
    ('bidir-h128-lr1e4', dict(lr=1e-4, wd=1e-4, step_every=6, seed=64, hidden=128, dropout=0.3)),
    ('bidir-h128-lr2e4', dict(lr=2e-4, wd=1e-4, step_every=6, seed=65, hidden=128, dropout=0.3)),
]
...
def train_cfg(name, cfg, X_tr, y_tr, X_va, y_va, X_ho, y_ho, run_base):
    ...
    model = AttentionLSTMModel(input_size=12, hidden_size=cfg['hidden'], dropout=cfg['dropout'],
    ...
    for epoch in range(EPOCHS):
        model, optimizer = quick_train(
            ...
            dropout=cfg['dropout'], hidden_size=cfg['hidden'], weight_decay=cfg['wd'],
            bidirectional=True)
    ...
    m = {'config': name, 'features': FEATURES_12, 'window': 90, 'hidden_size': cfg['hidden'],
         'bidirectional': True, 'dropout': cfg['dropout'], 'weight_decay': cfg['wd'],
         'lr': cfg['lr'], 'step_every': cfg['step_every'], 'seed': cfg['seed'],
    ...
    print(f"  {name}: val AUC={va['auc']:.4f} | holdout AUC={ho['auc']:.4f} (best {best_auc:.4f})", flush=True)
    return m
...
def main():
    ...
    for name, cfg in CONFIGS:
        train_cfg(name, cfg, X_tr, y_tr, X_va, y_va, X_ho, y_ho, run_base)
